...
pygame.display.set_caption("Galaxy_Game")
...
BULLET_HIT_SOUND = pygame.mixer.Sound("laser.wav")
BULLET_FIRE_SOUND = pygame.mixer.Sound("explosion.wav")


HEALTH_FONT = pygame.font.SysFont('centurygothic', 40)
WINNER_FONT = pygame.font.SysFont('centurygothic', 100)

FPS = 60
VEL = 5
BULLET_VEL = 7
MAX_BULLETS = 3
SPACESHIP_WIDTH, SPACESHIP_HEIGHT = 150, 135
...
RED_SPACESHIP_IMAGE = pygame.image.load(
    os.path.join('Assets', 'spaceship3.png'))
...
def yellow_handle_move(keys_pressed, yellow):
    ...
    if keys_pressed[pygame.K_UP] and yellow.y - VEL > 0: # UP
        yellow.y -= VEL
    if keys_pressed[pygame.K_DOWN] and yellow.y + VEL + yellow.height < HEIGHT +5: # DOWN
        yellow.y += VEL
...
def red_handle_move(keys_pressed, red):
    ...
    if keys_pressed[pygame.K_w] and red.y - VEL > 0: # UP
            red.y -= VEL
    if keys_pressed[pygame.K_s] and red.y + VEL + red.height < HEIGHT +5: # DOWN
            red.y += VEL
...
def main():
    ...
    while run:
        ...
        if red_health <= -1:
            winner_text = "Jtay Yellow Wins!"

        if yellow_health <= -1:
            winner_text = "Tay Red Wins!"
